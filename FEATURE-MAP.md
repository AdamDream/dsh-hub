...
| **subagent 模型 settings 默认层（dsh-tool-subagent P0' 补丁）** | 已实现·实测（2026-09-17；端到端双向真实派发：覆写→子代理实际跑 `glm-5.3`，清空→回退 `deepseek-v4.1-flash`） | [subagent-model-settings-exec.md](.workspace/subagent-model-settings-exec.md) · [deploy-subagent-model/dsh-tool-subagent.p0.diff](.workspace/deploy-subagent-model/dsh-tool-subagent.p0.diff) | 官方包 `dsh-tool-subagent` execute 在 `requestedAgentOptions` 合并前插入 settings 默认层：读 `dsh-subagent` 命名空间 provider/model（**settings 优先、preset agentOptions 兜底**；读失败降级静态路由不报错）；层序 = 工具显式选择(S21 休眠) > settings > preset > 父路由继承；非法值经 preflight 实时校验抛错。**宿主 lib 改动需重启一次生效**。 |
...
| **subagent 模型值级热载** | 已实现·**待重启后验收**（2026-09-17） | [subagent-model-settings-exec.md](.workspace/subagent-model-settings-exec.md) §热载语义 | 改 `settings.yaml` 的 `dsh-subagent:` 段（或设置页保存）→ **无需重启**，下一次 subagent / subagent_fork 派发即用新模型；段缺失/清空 → 回退 preset 默认。注意：**preset 文件本身（`~/.dsh/.agent-presets/*/agent.cordis.yml`）不热**——子代理派发走 `bindScopeParent` 复用父 standing mount，绕开 `ensureStanding` 的 stamp 重挂载检查（实测：改 preset 后 41s 派发的子代理仍用旧模型）；要热请改 settings 段。 |
