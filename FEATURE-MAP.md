...
| **dsh-subagent-model 子代理模型设置页** | 已实现·**已重启验收**（2026-09-17 部署；重启 17:45:39 后实测：boot 图含该插件、bundle HTTP 200/17099B 含「子代理模型」+ `settings.section` + `order: 70`、host 侧命名空间注册原文已核；GUI 目视待用户） | [acceptance-exec.md](.workspace/acceptance-exec.md) §2 P1-6 · [subagent-model-settings-exec.md](.workspace/subagent-model-settings-exec.md) · [deploy-subagent-model/README.md](.workspace/deploy-subagent-model/README.md) | 新插件 `@local/dsh-subagent-model`：host 注册 `dsh-subagent` settings 命名空间（provider/model 可选）+ 浏览器侧 settings.section「子代理模型」（order 70，下拉取自 `llm-pi-ai` 同源清单，不可用退化为文本输入）；默认 = preset 固定路由 `adam/deepseek-v4.1-flash`，清空即回退；保存走原子写 settings.yaml。 |
...
| **goal 修复（P0-A + 方案 A）** | 已实现·**已活体验收**（2026-09-17；首次真实观测：等待期零 goal 轮注入、唤醒源为 `subagent-settled` 通知；独立复核 271 行报告无未封堵路径） | [acceptance-exec.md](.workspace/acceptance-exec.md) §1 P0-4 · [verify-goal-gate.md](.workspace/acceptance-probe/verify-goal-gate.md) · [goal-p0a-exec.md](.workspace/goal-p0a-exec.md) | dsh-goal-round-driver：宿主 pause 中止运行中 round（keepInbox 不丢状态）+ 等待 subagent 期间空转注入修复（per-agent `pendingSubagents` 计数 + `competingQueued` 窄窗口兜底）。**R1 可见性加固（2026-09-17，仅日志、零行为变更）**：两处归因失败分支加 `ctx.logger.warn`，把唯一 fail-open 方向从静默变为可观测（sha `c4f3ea68…` → `4351e1742d3a…`）——**宿主 lib 属冷面，该加固待下一次重启才生效**。 |
| **subagent 模型 settings 默认层（dsh-tool-subagent P0' 补丁）** | 已实现·实测（2026-09-17；端到端双向真实派发：覆写→子代理实际跑 `glm-5.3`，清空→回退 `deepseek-v4.1-flash`） | [subagent-model-settings-exec.md](.workspace/subagent-model-settings-exec.md) · [deploy-subagent-model/dsh-tool-subagent.p0.diff](.workspace/deploy-subagent-model/dsh-tool-subagent.p0.diff) | 官方包 `dsh-tool-subagent` execute 在 `requestedAgentOptions` 合并前插入 settings 默认层：读 `dsh-subagent` 命名空间 provider/model（**settings 优先、preset agentOptions 兜底**；读失败降级静态路由不报错）；层序 = 工具显式选择(S21 休眠) > settings > preset > 父路由继承；非法值经 preflight 实时校验抛错。**宿主 lib 改动需重启一次生效**（已于 2026-09-17 17:45:39 重启加载；实测重启后所有子代理 `agentModel=deepseek-v4.1-flash`）。**S21 定性（2026-09-17 裁决：否决启用）**：`modelSelectionSettings: true` 会在装配期 `throw` —— 该开关要求的宿主半边模块（`dsh-tool-subagent` 的 `./model-selection-settings` export）在本版本树（0.1.1-rc.2）**不存在**（仅 0.1.5 归档树有）；且其语义（模型自选 provider/model）与「路由调整须用户明确指令」冲突。详见 [acceptance-exec.md](.workspace/acceptance-exec.md) §3.4。 |
...
| 项 | 状态 | 入口 | 一句话说明 |
| --- | --- | --- | --- |
| **context overflow 根因修复（adam 45 条 `contextWindow` 补齐）** | 已实现·**实测止血**（2026-09-17 18:04 热载，无需重启） | [acceptance-exec.md](.workspace/acceptance-exec.md) §3.6 · [rootcause-context-overflow.md](.workspace/acceptance-probe/rootcause-context-overflow.md) · [context-window-table.md](.workspace/acceptance-probe/context-window-table.md) | 根因：adam 50 个模型条目里 45 条未声明 `contextWindow` → 回落 `dsh-llm-pi-ai` 默认 **262144**，pi-ai 客户端预估把正常轮次末步误判 `CONTEXT_WINDOW_EXCEEDED`（`maxTokens` **不参与**溢出判定）。修复：按「证据优先保守档」补齐 40 条（E1 同文件条目 7 / E2-cat 本机 pi-ai 目录规格 28 / E3 兜底 1000000 5），5 条非对话模型（veo ×2 + gemini 图像变体 ×3）不填。实测：`request/context` 事件 17:46 `262144` → 18:04 `1000000`（同进程热载）；受压探测把 `deepseek-v4.1-flash` 真实窗口夹到 **(998,887, 1,055,800]**，与 2^20=1,048,576 吻合。**注意**：该网关对超窗请求返回的是 `500 get_channel_failed`（非长度错误文案），所以客户端窗口声明必须准。 |
| **vision-adam URL 拼接加固** | 已实现·实测（2026-09-17） | [acceptance-exec.md](.workspace/acceptance-exec.md) §3.7 · [fact-baseurl-slash.md](.workspace/acceptance-probe/fact-baseurl-slash.md) | 手写 `` fetch(`${options.baseURL}/chat/completions`) `` 加 `.replace(/\/+$/, "")` 归一化（部署位与源码副本逐字节一致，`node --check` 通过）。这是真正跑出过 `Invalid URL (POST /v1//chat/completions)` 的路径（失败工具 `analyze_image`；该文案实为网关 404 响应体）。 |
...
| **仓库瘦身 A 路线（停止跟踪 `.venv-ppt-test`）** | 已实现（2026-09-17） | [acceptance-exec.md](.workspace/acceptance-exec.md) §3.3 · [brief-history-bigfile.md](.workspace/acceptance-probe/brief-history-bigfile.md) | HEAD 曾跟踪 `.workspace/deploy-pptmaster/.venv-ppt-test/` = 1817 文件 / 51.62 MiB（占 HEAD 跟踪内容 52.8%，未 ignore），由 `3028e624` 误入库 → `git rm -r --cached` + `.gitignore`。历史不动（mgr.tgz 51.08 MiB 仍在历史，但「GitHub 持续告警」已证伪 = 09-15 单次 push 的一次性 GH001）。 |
| **btw 图像管线 D1/D2 修复** | 已实现·**待重启生效**（2026-09-18；类型检查通过、vitest 233 passed / 2 skipped，新增两条针对性测试） | [btw-image-pipeline-audit.md](.workspace/btw-image-pipeline-audit.md) · [usage-chart-restart-acceptance.md](.workspace/usage-chart-restart-acceptance.md) §4.3/§4.4 | **D1**：`imageRefsByMessageId` 原先只在活体 entry 收图时写入、resume 后拿到空 Map 且无重建路径 → 关抽屉再打开历史图缩略图消失、`readImage` 回 `Unknown attachment id`；修复为 resume 后从子会话日志的 `image` 内容块回填（`hydrateImageRefs`）。**D2**：侧聊工具白名单有 `read_image` 无 `analyze_image` → 补上（仍与父工具注册表取交集）。**诚实边界**：用户报告的"截图找不到"经审计**无法复现**（找到 3 次成功直传），D1 是最可能的表象来源。**"btw 贴图不产生 `request-images/` 派生文件"已结案（2026-09-18）**：不是管线差异，而是**请求预算**——pi-ai 路由 `maxBytes=1 MiB` / `maxPixels=2048²`，图已在预算内则原样直通、**只有需要重编码时才落盘**（`createRequestImage:634` + `readRequestImageFile:723`）；用户那张 5K 截图入库后仍 1.68 MiB 超预算→必有派生，btw 的 522 KB 图在预算内→无派生，主代理自己 `read_image` 看的 5 张同样无派生（行为级反证）。详见 [btw-request-images-explained.md](.workspace/btw-request-images-explained.md)。宿主 lib 属冷面 → 需重启一次。 |
