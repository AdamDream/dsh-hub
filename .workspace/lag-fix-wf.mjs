...
必读输入（先全部读完再动手）：
- 机制审计：/home/CNS2026495165/dsh/.workspace/lag-audit-mechanism.md（驱动=in-process、因果链、L3556-3574 mux 与 L1095-1127 FrameQueue 证据、修复点 §5-6）
- 差异审计：/home/CNS2026495165/dsh/.workspace/lag-audit-diff.md（回退完整、3 个丢失补丁清单与 tgz 位置、cordis.patch.yml 残留 disabled 条目）
- ②b 补丁权威记录：/home/CNS2026495165/dsh/execution-2b.md（两行改动的精确语义与复核 PASS）
...
- 配置：~/.dsh/settings.yaml（adam provider 的 models 段——deepseek-v4-flash 的 maxTokens 现值；vision-adam 段——model/baseURL/apiKeyEnv/maxTokens 现值）；以及已产出的 vision-adam 新 lib：/home/CNS2026495165/dsh/.workspace/deploy/vision-adam/lib/index.js 与其 settings 片段 .workspace/deploy/settings-vision-adam.snippet.yaml（默认 maxTokens=2000）

必须核实并给证据（文件:行号/路径）：
1. tgz 内 3 个补丁文件的确切内容与目标路径；与 execution-2b.md 描述的 ②b 改动是否一致（解包后 diff 核对）；与 live 原厂文件是否可干净合并（补丁上下文匹配或需重新锚定——给结论）。
...
4. 重放脚本规格：脚本应含 备份（cp -r/时间戳后缀）、应用（cp 或 patch）、校验（grep 锚点/node --check/字节比对）、幂等（已应用则跳过并提示）、dry-run 模式；覆盖 3 补丁恢复 + 2 处加固 + settings 片段写入；目标路径用 live 全局树；脚本放工作区 .workspace/deploy-lag/。
...
产出（必须）：
- 细粒度交付单元清单落盘 /home/CNS2026495165/dsh/.workspace/lag-fix-audit.md（每单元：编号 U-1..U-N、目标文件、精确改动、验收标准；含 tgz 补丁与 live 文件的合并方式结论、mux/FrameQueue 精确改法、settings 片段、重放脚本规格）
- 返回结构化结论（见 schema）。

约束：只读真实代码；不确定标「未确认」；不写任何代码（报告除外；tgz 解包到工作区临时目录做只读核对可以）；你继承当前沙箱模式（可写 /home/CNS2026495165/dsh），禁止使用 sandbox_permissions。`;

const EXEC_PROMPT = `你是「subagent 多开卡顿修复」三阶段闭环的【修订并执行】阶段子代理（路由 adam/deepseek-v4-flash）。职责：严格按审计交付单元产出全部修复产物到工作区 .workspace/deploy-lag/（部署位在 ~/.npm-global 全局树与 ~/.dsh/settings.yaml，属工作区外，由主代理部署期应用——你只产出文件与脚本，不改部署位）。不做设计决策、不扩范围；单元歧义在报告里客观描述，能落地的照做。

必读输入：
- 审计交付单元（权威）：/home/CNS2026495165/dsh/.workspace/lag-fix-audit.md
- 机制审计 / 差异审计：/home/CNS2026495165/dsh/.workspace/lag-audit-mechanism.md、lag-audit-diff.md；②b 记录：/home/CNS2026495165/dsh/execution-2b.md
...
产出（全部落在 /home/CNS2026495165/dsh/.workspace/deploy-lag/ 下）：
...
5. 执行报告 /home/CNS2026495165/dsh/.workspace/lag-fix-exec.md：产物清单+路径、逐单元状态、每项验证结果（node --check、grep 锚点、与 tgz 补丁字节一致、yaml 解析、bash -n）、遗留问题。
6. 返回结构化结论（见 schema）。

验证要求（全在工作区产物上做，不碰部署位）：
- 3 个恢复文件与 tgz 补丁语义一致（diff 核对改动点与 execution-2b.md 描述吻合）；
- hardening 副本 node --check 通过、锚点 grep 命中、改动仅限审计指定函数；
- settings 片段 yaml.safe_load 解析通过；
- 脚本 bash -n 通过。

约束：只写 /home/CNS2026495165/dsh/.workspace/deploy-lag/ 与报告文件；不得改 ~/.npm-global 与 ~/.dsh 任何文件；禁止使用 sandbox_permissions。`;
...
必读输入：
- 审计交付单元：/home/CNS2026495165/dsh/.workspace/lag-fix-audit.md
- 执行报告：/home/CNS2026495165/dsh/.workspace/lag-fix-exec.md
- 产物：/home/CNS2026495165/dsh/.workspace/deploy-lag/（restore/ hardening/ settings/ replay-lag-fix.sh）
...
复核清单：
1. 逐单元验收标准是否达成（读产物真实核对，不信报告声称）：3 个补丁恢复文件与 tgz 补丁语义一致（diff 关键锚点）、②b 与 execution-2b.md 语义逐条吻合；
...
if (audit.__agentFailure) {
  return { phase: 'audit', failed: true, error: String(audit.error ?? ''), next: '主代理裁决：审计两次失败，检查 .workspace/lag-fix-audit.md 是否部分落盘后决定重跑或人工接手' };
}
...
phase('修订并执行');
log('修订并执行：产出 restore/hardening/settings 产物与重放脚本到 .workspace/deploy-lag/');
