...
必读输入（先全部读完再动手）：
- 方案契约 v2：/home/CNS2026495165/dsh/.workspace/btw-upgrade-plan.md（裁决 R1-1..R1-10 与沿用项 R0-3/4/5，不得违背/重开）
- 现状审计：/home/CNS2026495165/dsh/.workspace/btw-upgrade-audit.md（269 行，全部带文件:行号证据）
- 实测证据：/home/CNS2026495165/dsh/.workspace/opencode-deepseek-v4-flash-probe.md（deepseek-v4.1-flash 实测可用 + 认证三头）、vision-flash-test.md、vision-exp-probe.md、glm53-flash-image-smoke.md
...
产出（必须）：
- 修订后的方案 + 细粒度交付单元清单，落盘 /home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-audit.md（每单元含验收标准与依赖；明确 U-G 是「btw 内实现」还是「官方包 patch 产物」并给证据；明确 U-H vision-adam 新 lib 的完整设计：字段、认证、导出、兼容）
- 返回结构化结论（见 schema）。
...
必读输入：
- 审计交付单元：/home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-audit.md（权威清单；你只负责其中标为 dsh-btw 源码的部分，不含部署产物）
- 方案契约 v2：/home/CNS2026495165/dsh/.workspace/btw-upgrade-plan.md（裁决 R1-1..R1-10，不得违背）
- 现状审计：/home/CNS2026495165/dsh/.workspace/btw-upgrade-audit.md；实测：/home/CNS2026495165/dsh/.workspace/opencode-deepseek-v4-flash-probe.md
...
5. 产出执行报告 /home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-exec.md：逐单元状态（done/部分/跳过+理由）、改动文件+关键行号、验证输出摘要（vitest 总数/通过数）、遗留问题。
6. 返回结构化结论（见 schema）。
...
必读输入：
- 审计交付单元：/home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-audit.md（其中 U-H vision-adam 配置化新 lib 的完整设计、以及 U-G 若审计判定「官方包 patch」时的补丁规格，是本线权威）
- 方案契约 v2：/home/CNS2026495165/dsh/.workspace/btw-upgrade-plan.md（R1-4/R1-5/R1-9）
- 实测证据：/home/CNS2026495165/dsh/.workspace/opencode-deepseek-v4-flash-probe.md（模型 deepseek-v4.1-flash、认证三头、请求格式）
- 现插件副本（只读参考）：~/.dsh/profiles/node_modules/@deepseek-ai/dsh-vision-adam/lib/index.js 与 package.json

产出（全部落在 /home/CNS2026495165/dsh/.workspace/deploy/ 下）：
1. vision-adam 配置化新 lib：.workspace/deploy/vision-adam/lib/index.js —— 严格按审计 U-H 设计实现（Config 扩展 model/baseURL/apiKeyEnv/maxTokens/认证头，默认 model=deepseek-v4.1-flash、baseURL=https://opencode.ai/zen/go/v1、apiKeyEnv=OPENCODE_GO_API_KEY；认证三头 authorization Bearer + x-api-key 同 key + x-opencode-session 随机 UUID；保留 analyze_image 工具注册与现有调用方兼容；若审计要求则导出可复用 analyze 函数）；同目录放变更说明 .workspace/deploy/vision-adam/CHANGES.md。用 node --check 验证语法（若因 ESM 无法 --check，用 node -e 动态 import 前先说明风险或跳过并记录）。
2. 若审计判定 U-G 需「官方包 patch」：产出补丁文件到 .workspace/deploy/patches/（被改文件的新版本或 unified diff，标明目标绝对路径与备份/应用步骤），对象为 dsh-agent-loop 或 dsh-host-apiproxy 的消息管线最小改动；若审计判定 U-G 属 btw 内实现，则本线跳过此项并在报告注明。
3. settings.yaml 的 vision-adam 段最终形态：.workspace/deploy/settings-vision-adam.snippet.yaml（含注释，兼容旧键）。
4. 产出执行报告 /home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-exec-patches.md：产物清单+路径、与审计 U-H/U-G 规格的逐条对应、node --check 结果、已知风险（如现有调用方兼容点）。
5. 返回结构化结论（见 schema）。

约束：只写 /home/CNS2026495165/dsh/.workspace/deploy/ 与报告文件（工作区内）；不得改 ~/.dsh 任何文件；禁止使用 sandbox_permissions。`;
...
必读输入：
- 方案契约 v2：/home/CNS2026495165/dsh/.workspace/btw-upgrade-plan.md
- 审计交付单元：/home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-audit.md
- 执行报告：/home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-exec.md 与 /home/CNS2026495165/dsh/.workspace/btw-upgrade-impl-exec-patches.md
- 真实代码：/home/CNS2026495165/dsh/dsh-btw/（逐单元核对；工作区根有 git 则用 git -C /home/CNS2026495165/dsh status/diff 查看）；部署产物：/home/CNS2026495165/dsh/.workspace/deploy/
...
if (audit.__agentFailure) {
  return { phase: 'audit', failed: true, error: String(audit.error ?? ''), next: '主代理裁决：审计两次失败，检查 .workspace/btw-upgrade-impl-audit.md 是否部分落盘后决定重跑或人工接手' };
}
