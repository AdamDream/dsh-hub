...
export class SerialSessionManager {
    ...
    send(data, metadata) {
        if (data.byteLength === 0)
            throw new TypeError('serial write must contain at least one byte');
    ...
    mark(label, actor, toolCallId) {
        const normalized = label.trim();
        if (normalized.length === 0)
            throw new TypeError('serial marker label must not be blank');
        return this.publish({
            type: 'marker',
            actor,
            label: normalized,
    ...
    snapshot(afterSeq = 0, limit = this.snapshotLimit) {
    ...
    waitSnapshot(request = {}, signal) {
        if (this.closed)
            return Promise.reject(new SerialSessionManagerClosedError());
        ...
        const takeSnapshot = () => this.snapshot(request.afterSeq ?? 0, request.limit ?? this.snapshotLimit);
        const initial = takeSnapshot();
        if (waitMs === 0 || snapshotHasChanges(initial))
        ...
        return new Promise((resolve, reject) => {
            ...
            const onAbort = () => { rejectWith(abortReason(signal)); };
            const onClose = (error) => { rejectWith(error); };
    ...
    waitForText(request, signal) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const check = () => {
                ...
                if (match === null || this.sessionId === undefined || settled)
                    return;
                settled = true;
                cleanup();
                resolve({
    ...
    async close() {
        ...
        for (const closeWaiter of [...this.snapshotWaitClosers])
            closeWaiter(closeError);
    ...
    receive(data) {
        if (data.byteLength === 0 || this.sessionId === undefined)
            return;
        const bytes = Uint8Array.from(data);
    ...
    onTransportClosed() {
        if (this.intentionalClose || this.transport === undefined)
            return;
        this.detachTransport();
    ...
    publish(body) {
        if (this.sessionId === undefined)
            throw new Error('cannot publish a serial event without a session');
        const event = {
            ...body,
            sessionId: this.sessionId,
            seq: this.nextSeq++,
...
function snapshotHasChanges(snapshot) {
...
function validateOpenOptions(options) {
    if (options.path.trim().length === 0)
        throw new TypeError('serial path must not be blank');
