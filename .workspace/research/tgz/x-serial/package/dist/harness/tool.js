...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: `Serial connection ${value.status}` }],
        },
        async execute(args) {
            return snapshotView(await ctx.serialConsole.connect({
                path: args.port,
                baudRate: args.baud_rate,
                ...(args.data_bits === undefined ? {} : { dataBits: args.data_bits }),
                ...(args.stop_bits === undefined ? {} : { stopBits: args.stop_bits }),
    ...
    ctx.tools.register(defineTool({
        ...
        execute(args) {
            ...
            if (lines.length > maxReadLines)
                text = lines.slice(-maxReadLines).join('\n');
            const encoder = new TextEncoder();
            while (text.length > 0 && encoder.encode(text).byteLength > maxReadBytes)
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: value.match }],
        },
        async execute(args, execution) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: `marked ${value.label} (serial seq ${value.seq})` }],
        },
