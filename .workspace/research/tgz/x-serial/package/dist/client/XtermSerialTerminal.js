...
import { takeRestorableTerminalCheckpoint, saveTerminalCheckpoint, } from './terminal-checkpoint.js';
import { advanceTerminalTransmit, findTerminalSubmissionMatch, isReplayingTerminalEvent, isTerminalGeneratedReply, mapTerminalInput, } from './terminal-transmit.js';
/**
 ...
 */
export function XtermSerialTerminal({ events, connected, follow, lineEnding, emptyLabel, checkpointKey, checkpointBaseSeq, checkpointAllowed, checkpointCache, findOpen, onFindOpenChange, onTextInput, onBinaryInput, }) {
    ...
    const checkpointCandidateRef = useRef();
    ...
    const [restoring, setRestoring] = useState(checkpointCandidate !== null || replayThroughSeqRef.current > checkpointBaseSeq);
    const [gutterRows, setGutterRows] = useState([]);
    ...
    useEffect(() => {
        ...
        const dataDisposable = terminal.onData(data => {
            if (runtime.restoring || !connectedRef.current || data.length === 0)
                return;
            if (isReplayingTerminalEvent(runtime.replayThroughSeq, runtime.processingRxSeq)
                && isTerminalGeneratedReply(data))
                return;
        ...
        let renderDisposable;
        let scrollDisposable;
        let resizeDisposable;
        let resizeObserver;
        let initialResizeFrame;
        const resize = () => {
            if (runtime.disposed || host.clientWidth === 0 || host.clientHeight === 0)
                return;
            fitAddon.fit();
            refresh();
        ...
        const finishCheckpointRestore = () => {
            if (runtime.disposed || checkpoint === null)
                return;
        ...
        if (checkpoint === null) {
            ...
            try {
                if (checkpoint.payload.serializedTerminal === '')
                    finishCheckpointRestore();
                else
                    terminal.write(checkpoint.payload.serializedTerminal, finishCheckpointRestore);
            }
            catch {
                replayWithoutCheckpoint();
            }
        ...
        return () => {
            ...
            if (runtime.checkpointTimer !== undefined)
                clearTimeout(runtime.checkpointTimer);
            if (runtime.revealFrame !== undefined)
    ...
    useEffect(() => {
        ...
        if (restoring)
            return;
        if (searchTerm !== '')
            runTerminalSearch(runtime.searchAddon, searchTerm, 'next', true);
    ...
    useEffect(() => {
        ...
        for (const event of events) {
            if (event.seq <= runtime.lastQueuedSeq)
                continue;
    ...
    const updateSearchTerm = (term) => {
        setSearchTerm(term);
        const runtime = runtimeRef.current;
        if (runtime === undefined || restoring)
            return;
        if (term === '') {
            clearTerminalSearch(runtime);
            setSearchResults(EMPTY_SEARCH_RESULTS);
    ...
    const navigateSearch = (direction) => {
        const runtime = runtimeRef.current;
        if (runtime === undefined || restoring || searchTerm === '')
            return;
    ...
    return (_jsxs("div", { className: `dsh-serial-terminal-stage${restoring ? ' is-restoring' : ''}`, role: "application", "aria-label": "Serial VT terminal", "aria-busy": restoring, children: [_jsx("div", { className: "dsh-serial-gutter", "aria-label": "Terminal source markers", children: gutterRows.map(row => (_jsx("span", { className: `dsh-serial-gutter-row is-${row.actor}`, style: { top: `${row.top}px` }, title: gutterTitle(row.actor), children: gutterLabel(row.actor) }, row.id))) }), _jsx("div", { ref: hostRef, className: "dsh-serial-xterm-host" }), findOpen && !restoring && (_jsxs("div", { className: "dsh-serial-find", role: "search", "aria-label": "Find in terminal", children: [_jsx("input", { ref: searchInputRef, type: "search", value: searchTerm, placeholder: "Find", "aria-label": "Find text in terminal", autoComplete: "off", spellCheck: false, onChange: event => { updateSearchTerm(event.target.value); }, onKeyDown: handleSearchKeyDown }), _jsx("output", { className: "dsh-serial-find-count", "aria-live": "polite", children: searchResultLabel }), _jsx("button", { type: "button", "aria-label": "Previous match", title: "Previous match (Shift+Enter)", disabled: searchTerm === '', onClick: () => { navigateSearch('previous'); }, children: "\u2191" }), _jsx("button", { type: "button", "aria-label": "Next match", title: "Next match (Enter)", disabled: searchTerm === '', onClick: () => { navigateSearch('next'); }, children: "\u2193" }), _jsx("button", { type: "button", "aria-label": "Close find", title: "Close (Escape)", onClick: () => { onFindOpenChange(false); }, children: "\u00D7" })] })), restoring && _jsx("div", { className: "dsh-serial-terminal-restoring", children: "Restoring terminal\u2026" }), !restoring && events.length === 0 && _jsx("div", { className: "dsh-serial-terminal-hint", children: emptyLabel })] }));
}
...
export function runTerminalSearch(searchAddon, term, direction, incremental) {
    const options = {
        caseSensitive: false,
...
function clearTerminalSearch(runtime) {
    ...
    if (runtime.terminal.element !== undefined)
...
function drainEvents(runtime, followRef, setRows) {
    if (!runtime.readyToDrain || runtime.draining || runtime.disposed)
        return;
    ...
    if (event.type === 'rx') {
        ...
        runtime.terminal.write(decodeBase64(event.dataBase64), () => {
            ...
            if (!runtime.disposed) {
                const span = describeReceiveSpan(event.seq, startLine, currentLine(runtime.terminal));
                ...
                if (runtime.receiveTail.length > 16)
                    runtime.receiveTail.shift();
    ...
    if (event.type === 'tx')
        observeTransmit(runtime, event, refresh);
    else {
        runtime.receiveTail.splice(0);
...
function scheduleCheckpoint(runtime) {
    if (runtime.disposed || runtime.restoring || !runtime.checkpointAllowed)
        return;
    if (runtime.checkpointTimer !== undefined)
        clearTimeout(runtime.checkpointTimer);
...
function saveRuntimeCheckpoint(runtime) {
    if (runtime.disposed
        || runtime.restoring
        || runtime.draining
        ...
        || runtime.terminal.buffer.active.type !== 'normal')
        return;
    ...
    if (existing?.key === runtime.checkpointKey
        ...
        && existing.rows === runtime.terminal.rows)
        return;
...
function restoreCheckpointState(runtime, checkpoint, refresh) {
    ...
    runtime.pendingSubmissions.splice(0, runtime.pendingSubmissions.length, ...checkpoint.pendingSubmissions.map(pending => ({ ...pending })));
    runtime.receiveTail.splice(0, runtime.receiveTail.length, ...checkpoint.receiveTail.map(span => ({ ...span })));
    for (const cached of checkpoint.records) {
        const marker = runtime.terminal.registerMarker(cached.line - currentLine(runtime.terminal));
        if (marker === undefined)
            continue;
        const record = { marker, actor: cached.actor };
        runtime.records.push(record);
        marker.onDispose(refresh);
    }
}
function resetCheckpointState(runtime) {
    for (const record of runtime.records)
        record.marker.dispose();
...
function markReceiveSpan(runtime, span, refresh) {
...
function describeReceiveSpan(eventSeq, startLine, endLine) {
    return { eventSeq, startLine, endLine };
}
function observeTransmit(runtime, event, refresh) {
    ...
    if (runtime.pendingSubmissions.length > 128)
        runtime.pendingSubmissions.splice(0, 64);
}
function observeTransmitText(runtime, actor, text) {
...
function combineReceiveTail(spans) {
    const first = spans[0];
    const last = spans.at(-1);
    if (first === undefined || last === undefined)
        return undefined;
    return describeReceiveSpan(last.eventSeq, first.startLine, last.endLine);
}
function markBoardLine(runtime, line, refresh, force) {
    ...
    if (force)
        clearLineMarkers(runtime, line, refresh);
}
function clearLineMarkers(runtime, line, refresh) {
    for (const record of [...runtime.records]) {
        if (!record.marker.isDisposed && record.marker.line === line)
            record.marker.dispose();
    }
    runtime.records.splice(0, runtime.records.length, ...runtime.records.filter(record => !record.marker.isDisposed));
    refresh();
}
function markCorrelatedSubmission(runtime, span, submission, actor, refresh) {
    ...
    for (let line = Math.min(span.endLine, activeStart - 1); line >= span.startLine; line -= 1) {
        if (hasCommandAttribution(runtime, line))
            continue;
        ...
        if (!submission.opaque
            ...
            && !text.endsWith(submission.command))
            continue;
...
function resolvePendingSubmissions(runtime, refresh) {
    ...
    for (let index = 0; index < runtime.pendingSubmissions.length;) {
        ...
        if (line === undefined) {
            if (runtime.lastQueuedSeq - pending.txSeq > 512)
...
function findPendingSubmissionLine(runtime, pending, claimed) {
...
function markLine(runtime, line, actor, refresh, force = false) {
    if (runtime.terminal.buffer.active.type !== 'normal')
        return;
    runtime.records.splice(0, runtime.records.length, ...runtime.records.filter(record => !record.marker.isDisposed));
    const existing = runtime.records.find(record => record.marker.line === line);
    if (existing !== undefined) {
        if (force || actorPriority(actor) >= actorPriority(existing.actor))
            existing.actor = actor;
        refresh();
        return;
    }
    const marker = runtime.terminal.registerMarker(line - currentLine(runtime.terminal));
    if (marker === undefined)
        return;
    const record = { marker, actor };
    runtime.records.push(record);
    marker.onDispose(refresh);
    refresh();
}
function refreshGutter(runtime, setRows) {
    if (runtime.disposed)
        return;
    ...
    for (const record of runtime.records) {
        if (record.marker.isDisposed || record.marker.line < buffer.viewportY)
            continue;
        if (record.marker.line >= buffer.viewportY + runtime.terminal.rows)
            continue;
        if (record.marker.line >= activeStart && record.marker.line <= activeEnd)
            continue;
    ...
    if (signature === runtime.gutterSignature)
        return;
    runtime.gutterSignature = signature;
    setRows(rows);
}
function currentLine(terminal) {
    return terminal.buffer.active.baseY + terminal.buffer.active.cursorY;
}
function activeLogicalLineStart(terminal) {
    let line = currentLine(terminal);
    while (line > 0 && terminal.buffer.active.getLine(line)?.isWrapped === true)
        line -= 1;
    return line;
}
function terminalTheme(styles) {
    return {
...
function requiredVariable(styles, name) {
    const value = styles.getPropertyValue(name).trim();
    if (value === '')
        throw new Error(`Missing serial terminal CSS variable ${name}`);
    return value;
}
...
function actorPriority(actor) {
    if (actor === 'user' || actor === 'model')
        return 3;
    if (actor === 'system')
        return 2;
    return 1;
}
function gutterLabel(actor) {
    if (actor === 'user')
        return 'U';
    if (actor === 'model')
        return 'M';
...
//# sourceMappingURL=XtermSerialTerminal.js.map