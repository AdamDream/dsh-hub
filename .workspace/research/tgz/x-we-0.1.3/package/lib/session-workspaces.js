...
function canonicalLocalPath(value) {
    ...
    for (;;) {
        try {
            ...
            if (parent === head)
                return absolute; // no existing ancestor (unreachable drive)
 ...
 * one key.
 */
export function normalizeSideRootKey(kind, path) {
    if (typeof path !== 'string' || path.trim() === '')
        return null;
    const value = path.trim();
    if (kind === 'remote')
        return normalizeRemoteKey(value);
    if (!isAbsolute(value))
        return null;
 ...
 */
export function remoteSideRootKey(path) {
...
export function sideRootKeyOf(rootKey) {
 ...
 */
function isUnderRoot(root, candidate, insensitive) {
...
function bestMatchingRoot(roots, matches) {
    let best;
    let bestLength = -1;
    for (const item of roots.values()) {
        if (!matches(item.rootKey))
            continue;
 ...
 */
export function sideWorkspaceOf(roots, path) {
    if (typeof path !== 'string' || path.trim() === '')
        return undefined;
    const value = path.trim();
    ...
    if (process.platform === 'win32' && value.startsWith('/') && !value.startsWith('//')) {
        ...
        for (const item of roots.values()) {
            ...
            if (parsed === null || parsed.kind !== 'remote')
                continue;
            // Longest by PATH part: the machine id length must not decide the winner.
            if (!isUnderRoot(parsed.path, normalized, false))
                continue;
...
export function normalizeSideWorkspaceRecord(raw) {
    if (typeof raw !== 'object' || raw === null)
        return null;
    const record = raw;
    if (typeof record.id !== 'string' || record.id === '')
        return null;
    if (record.kind !== 'local' && record.kind !== 'remote')
        return null;
    if (typeof record.rootKey !== 'string')
        return null;
    ...
    try {
        ...
        if (typeof parsed.roots === 'object' && parsed.roots !== null) {
            for (const record of Object.values(parsed.roots)) {
                ...
                if (item !== null && !roots.has(item.rootKey))
                    roots.set(item.rootKey, item);
            }
        }
        if (typeof parsed.sessions === 'object' && parsed.sessions !== null) {
            for (const [sessionId, list] of Object.entries(parsed.sessions)) {
                if (typeof sessionId !== 'string' || sessionId === '' || !Array.isArray(list))
                    continue;
                ...
                for (const entry of list) {
                    ...
                    if (canonical === null || !roots.has(canonical) || keys.includes(canonical))
                        continue;
                    keys.push(canonical);
                }
                if (keys.length > 0)
                    sessions.set(sessionId.trim(), keys);
            }
        }
    }
    catch (error) {
...
export function allocateSideId() {
...
export class SessionSideWorkspaceStore extends Service {
    ...
    constructor(ctx, opts) {
        ...
        for (const [key, item] of state.roots)
            this.roots.set(key, item);
        for (const [sessionId, keys] of state.sessions)
    ...
    detach(sessionId, rootKey) {
        const sid = sessionId.trim();
        const list = this.sessions.get(sid);
        if (list === undefined || !list.includes(rootKey))
            return false;
    ...
    persist() {
        const roots = {};
        for (const [key, item] of this.roots)
            roots[key] = item;
        const sessions = {};
        for (const [sessionId, keys] of this.sessions)
...
export const sessionWorkspacesFilePath = (dshBase) => defaultSideWorkspacesFile(dshBase);
//# sourceMappingURL=session-workspaces.js.map