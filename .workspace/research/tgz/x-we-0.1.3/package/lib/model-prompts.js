...
export const MODEL_PROMPTS = {
    ...
    sideNote: 'Note the permission markers: read-only (fs: read-only) rejects writes, and execution disabled (exec: off) rejects running commands under that directory; for rejected operations use a workspace with permission or ask the user to adjust. Commands run in the main workspace by default; to run on another server use `sw_exec(server, command)`.',
    ...
    sectionSwExec: "sw_exec executes a command on the specified server; workdir defaults to that server's primary workspace. Check the [exit code: N] marker of each result; investigate non-zero exits before continuing.",
