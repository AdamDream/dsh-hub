...
import { parseSshTargetKey, remoteRouteFromCwd, sshTargetKey } from "./transport.js";
...
export class MixedFileSystem {
    ...
    lstat(path, opts, signal) {
        ...
        if (side !== undefined) {
            if (side.kind === 'remote') {
                const sideRoute = parseSshRoute(path);
                if (sideRoute !== null)
                    return this.remote.lstat(sideRoute.path, { cwd: side.rootKey }, signal);
            }
            else {
    ...
    async readByteRange(target, range, signal) {
        ...
        if (reader === undefined) {
            throw new FsError(`cannot read "${target.displayPath}": the local filesystem backend does not support windowed reads (dsh-fs before the 0.1.5 line)`, 'FS_IO_ERROR');
        }
