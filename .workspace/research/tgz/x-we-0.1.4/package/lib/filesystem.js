...
export class SshFileSystemEngine {
    ...
    async lstat(path, opts, signal) {
        ...
        try {
            const stats = await new Promise((resolve, reject) => {
                sftp.lstat(displayPath, (error, value) => { if (error !== undefined)
                    reject(error);
                else
                    resolve(value); });
            });
            assertNotAborted(signal, 'lstat');
            const type = stats.isSymbolicLink() ? 'symlink' : stats.isFile() ? 'file' : stats.isDirectory() ? 'directory' : 'other';
            return {
    ...
    async listDir(target, signal) {
        ...
        try {
            const listed = await new Promise((resolve, reject) => {
                sftp.readdir(route.path, (error, value) => { if (error !== undefined)
                    reject(error);
                else
                    resolve(value); });
            });
    ...
    ssh() {
        ...
        if (value === undefined)
            throw new Error('fs-ssh: the ssh transport is not mounted');
        return value;
    }
    async canonicalPath(path, signal, transport = this.ssh()) {
        const result = await transport.exec(`set -o pipefail; realpath -mz -- ${quoteShellArg(path)} | base64 -w0`, signal !== undefined ? { signal } : undefined);
        signal?.throwIfAborted();
        if (result.exitCode !== 0)
            throw new Error(result.stderr || `realpath failed for ${path}`);
        return decodeCanonicalPath(result.stdout.trim());
    }
    async probe(path, displayPath, signal, transport = this.ssh()) {
        assertNotAborted(signal, 'stat');
    ...
    async readBytesRaw(target, signal, maxBytes) {
        ...
        try {
            const data = await new Promise((resolve, reject) => {
                sftp.readFile(route.path, (error, value) => { if (error !== undefined)
                    reject(error);
                else
                    resolve(value); });
            });
    ...
    async writeAtomic(target, content, existing, createIfAbsent, signal) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                sftp.mkdir(stagingDirectory, error => { if (error !== undefined)
                    reject(error);
                else
                    resolve(); });
            });
            stagingCreated = true;
            await new Promise((resolve, reject) => {
                sftp.writeFile(temporary, content, error => { if (error !== undefined)
                    reject(error);
                else
                    resolve(); });
            });
            ...
            assertNotAborted(signal, 'write');
            if (createIfAbsent) {
                const publication = await route.transport.exec(`if ln -T -- ${quoteShellArg(temporary)} ${quoteShellArg(targetPath)}; then printf created; elif test -e ${quoteShellArg(targetPath)} || test -L ${quoteShellArg(targetPath)}; then printf exists; else exit 1; fi`, signal !== undefined ? { signal } : undefined);
    ...
    async removeStaging(directory, transport = this.ssh()) {
 ...
 * `mv -f`, quoted safely.
 */
async function overwritePublication(transport, temporary, target, signal) {
    try {
    ...
    const publication = await transport.exec(overwritePublicationCommand(temporary, target), signal !== undefined ? { signal } : undefined);
...
export function isOwnStagingDirectory(directory) {
...
export function overwritePublicationCommand(temporary, target) {
...
export function stagingCleanupCommand(directory) {
...
export class SshFileSystem extends FileSystem {
    ...
    writeText(target, content, expected, signal, _sandboxPolicy) {
        return this.engine.writeText(target, content, expected, signal);
    }
    editText(target, edit, expected, signal, _sandboxPolicy) {
