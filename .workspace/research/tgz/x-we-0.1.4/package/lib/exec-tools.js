...
import { hasRemoteSessionContext, hasRemoteWorkspaceContext, sessionIdOf } from "./session-remote-context.js";
/**
 ...
 */
export async function probeMachine(registry, id, budgetMs = MACHINE_PROBE_TIMEOUT_MS) {
    ...
    try {
        ...
        if (outcome.exitCode === 0)
            return { id, ok: true, latencyMs, detail: '' };
        return {
            id,
            ok: false,
            latencyMs,
...
export async function probeMachines(registry, ids, budgetMs = MACHINE_PROBE_TIMEOUT_MS) {
    const results = [];
    for (const id of ids)
        results.push(await probeMachine(registry, id, budgetMs));
    return results;
}
/**
 ...
 */
export function connectedMachinesOf(sessionCwd, sessionId, listFor, registry) {
 ...
 */
export function requireConnectedServer(connected, requested, registered, tr = EN_T) {
    if (connected.sessionId === null)
        throw new Error(tr('tool.sw_exec.error.noSession'));
    if (connected.ids.length === 0)
        throw new Error(tr('tool.sw_exec.error.notConnectedNone'));
    if (requested === undefined)
        return;
    if (connected.ids.includes(requested))
        return;
    if (registered.includes(requested)) {
        throw new Error(tr('tool.sw_exec.error.notConnected', { id: requested, ids: connected.ids.join(', ') }));
    }
    throw new Error(tr('tool.sw_exec.error.unknownServer', { id: requested })
    ...
    const tool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (connections !== undefined) {
                connected = connectedMachinesOf(sessionCwd, sessionIdOf({ ...(exec.agent !== undefined ? { scope: exec.agent } : {}) }), sessionId => connections()?.listFor(sessionId) ?? [], registry());
            }
    ...
    const tool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (connectionStore !== undefined && route !== null) {
                const connected = connectedMachinesOf(sessionCwdOf(exec), sessionIdOf({ ...(exec.agent !== undefined ? { scope: exec.agent } : {}) }), sessionId => connectionStore.listFor(sessionId), registry());
