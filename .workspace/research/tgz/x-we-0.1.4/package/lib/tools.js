...
import { probeMachines, registerSwExec, registerWin32Bash } from "./exec-tools.js";
import { lookup } from "./locale/index.js";
import { hostLocaleOf, localizeTool } from "./locale/host.js";
import { modelPrompt } from "./model-prompts.js";
import { sessionIdOf, sessionWorkspaceContextOf } from "./session-remote-context.js";
 ...
 */
export function renderRemotePrompt(fact) {
...
export function sideWorkspacePromptFact(item) {
    return {
        label: item.label,
 ...
 */
export function renderSideWorkspaces(items) {
    if (items.length === 0)
        return '';
...
export function connectedMachineFact(id, machine) {
 ...
 */
export function renderConnectedMachines(facts) {
    if (facts.length === 0)
        return '';
...
export function renderConnectedStatusLine(ids, machineOf, tr) {
    if (ids.length === 0)
        return tr('tool.sw_status.outputs.connNone');
    const items = ids
 ...
 */
export function renderSessionWorkspaceContext(connected, sides) {
    const parts = [];
 ...
 */
export function composeWorkspacePrompt(cwd, machine, sides, dshBase, connected = []) {
 ...
 */
export function renderRemoteEnvProbe(probe) {
...
async function remoteEnvLine(registry) {
    const active = registry.getActive();
    if (active === null)
        return '';
    try {
...
function sessionIdOfExec(exec) {
    return sessionIdOf(exec.agent !== undefined ? { scope: exec.agent } : {});
}
/** The session cwd the tools read (leaf field only). */
function sessionCwdOfExec(exec) {
 ...
 */
export function registerWorkspaceTools(ctx, registry, sides, connections) {
    const locale = hostLocaleOf(ctx);
    const t = locale.t;
    const machineFaceOf = (id) => registry().listMachines().machines.find(entry => entry.id === id);
    /** REQ-I11: the session's connected ids (store ∪ implicit main machine). */
    const connectedIdsOf = (sessionId, cwd) => {
    ...
    const connectedFactsOf = (sessionId, cwd) => connectedIdsOf(sessionId, cwd).map(id => connectedMachineFact(id, machineFaceOf(id)));
    const tools = [
        ...
        localizeTool(defineTool({
            ...
            async execute(args, exec) {
                const sessionId = sessionIdOfExec(exec);
                if (sessionId === undefined)
                    throw new Error(t('tool.sw_connect.error.noSession'));
                const store = connections();
                if (store === undefined)
                    throw new Error(t('tool.sw_connect.error.storeMissing'));
                const instance = registry();
                const known = instance.listMachines().machines.map(machine => machine.id);
                const requested = [...new Set((Array.isArray(args.machines) ? args.machines : [])
