...
import { normalizeRemoteApproval } from "./remote-approval-gate.js";
import { normalizeRemoteSandbox } from "./remote-sandbox.js";
...
export function normalizeMachine(raw) {
    ...
    if (remoteApproval !== 'off')
        machine.remoteApproval = remoteApproval;
    ...
    if (remoteSandbox !== 'off')
        machine.remoteSandbox = remoteSandbox;
...
export class SshRegistry extends Service {
    ...
    machineViewOf(spec) {
        ...
        return {
            ...
            remoteSandbox: normalizeRemoteSandbox(spec.remoteSandbox),
    ...
    applyInputFieldsInto(spec, input) {
        ...
        if (input.remoteApproval !== undefined) {
            ...
            if (mode === 'off')
                delete spec.remoteApproval;
            else
                spec.remoteApproval = mode;
        }
        ...
        if (input.remoteSandbox !== undefined) {
            ...
            if (mode === 'off')
                delete spec.remoteSandbox;
            else
                spec.remoteSandbox = mode;
        }
    }
}
