...
import { SessionMachineConnections, normalizeMachineIds } from "./session-connections.js";
import { normalizeSideRootKey, remoteSideRootKey } from "./session-workspaces.js";
import { hostLocaleOf } from "./locale/host.js";
import { channelRouteOf, isAlreadyRegistered } from "./web-channel.js";
...
function isMachineInput(value) {
    ...
    if (value.remoteApproval !== undefined
        && value.remoteApproval !== 'off' && value.remoteApproval !== 'human' && value.remoteApproval !== 'ai')
        return false;
    ...
    if (value.remoteSandbox !== undefined
        && value.remoteSandbox !== 'off' && value.remoteSandbox !== 'read-only' && value.remoteSandbox !== 'workspace-write')
        return false;
    return true;
}
...
function isSessionConnIdPayload(value) {
...
function isSessionConnListPayload(value) {
...
function isSessionConnSetPayload(value) {
    if (!isRecord(value))
        return false;
    if (!isString(value.sessionId) || value.sessionId.trim() === '')
        return false;
...
export function apply(ctx, config) {
     ...
     */
    const connStore = () => {
        const value = ctx.get('sessionConnections', false);
        if (value === undefined)
            throw new Error(`dsw: ${t('rpc.connStoreNotMounted')}`);
        return value;
    };
    /** REQ-I11: every named machine must be in the registry (the machine universe). */
    const requireRegisteredMachines = (ids) => {
    ...
    const connItems = (sessionId) => ({ items: [...connStore().listFor(sessionId)] });
        ...
        try {
            switch (endpoint) {
                ...
                case 'session.conn.connect': {
                    ...
                    if (id === undefined)
                        throw new Error(`dsw: ${t('rpc.connMachineEmpty')}`);
    ...
    registerWorkspaceTools(ctx, registry, () => ctx.get('sideWorkspaces', false), () => ctx.get('sessionConnections', false));
}
