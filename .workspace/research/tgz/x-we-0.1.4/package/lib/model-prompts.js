...
export const MODEL_PROMPTS = {
    ...
    sideNote: 'A side workspace is an extra directory this session can operate on directly. Commands run in the main workspace by default; to run a command on another server use `sw_exec(server, command)`.',
    /**
    ...
    envMissing: 'Hint: the remote is missing {missing} — install them on the remote (for reference only; not auto-installed): rg → sudo apt-get install ripgrep; pwsh → https://aka.ms/powershell',
    /**
     ...
     */
    sectionSwExec: "sw_exec executes a command on the specified server. The server must be connected to this session: use an id from the connected-machine list, or call sw_connect first. workdir defaults to that server's primary workspace. Check the [exit code: N] marker of each result; investigate non-zero exits before continuing.",
    /** `tool:bash` section (order 105, win32 hosts) — injected only in a remote-context session. */
    sectionWin32Bash: 'The bash tool targets remote Linux workspaces; use pwsh for local (Windows) sessions. Check the [exit code: N] marker of each result.',
    /**
     ...
     * (ADR-0020 D6).
     */
    remoteNoSandbox: 'Remote execution is not confined by the local sandbox: commands run with the remote OS account\'s permissions only.',
    /**
     ...
     */
    remoteGateActive: 'Commands on this machine additionally require an approval decision before they run; do not retry a rejected command unchanged.',
    /**
 ...
 */
export function modelPrompt(key, params) {
    const template = MODEL_PROMPTS[key];
    if (params === undefined)
        return template;
    return template.replace(/\{(\w+)\}/g, (match, name) => (name in params ? String(params[name]) : match));
}
//# sourceMappingURL=model-prompts.js.map