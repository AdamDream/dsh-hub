...
function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    name: "analyze_image",
    description: "Analyze a PNG/JPEG/WebP/GIF image file or an MP4/M4V/WebM/MOV/MKV/AVI video file with a multimodal model and return a text description or answer. Returns text only, so it works with ANY model — including text-only models that cannot receive images directly. Use this whenever you need to understand what is in an image or video.",
    parameters: {
      file_path: {
        type: "string",
        required: true,
