...
describe('package contract', () => {
  it('publishes all Typert remotes', () => {
    expect(sideChatRemoteDescriptors.map(item => item.method)).toEqual(['start', 'read', 'send', 'answer', 'cancel', 'close'])
    expect(new Set(sideChatRemoteDescriptors.map(item => item.id)).size).toBe(6)
