...
(() => {
  ...
  const counters = {
    ...
    tcRafScheduled: 0, tcRafScheduledInClick: 0, tcSyncInvoked: 0, tcRafReentrant: 0,
  ...
  if (COND === "tp-off" || COND === "tpoff-armed") {
  ...
  // ⚠️ 现场教训：初版无条件同步执行回调 ⇒ 生产代码里存在「rAF 自续期」循环
  //    （实测 window.requestAnimationFrame 在 tick 内再次调用自己），
  //    同步化立刻变成无限递归 → `RangeError: Maximum call stack size exceeded`。
  //    修法：**只在非重入时同步执行**，重入的调用退回真实 rAF（仍满足「本帧内写回」的语义，
  //    因为第一次调用已经同步跑过），同时记录重入次数作为「存在 rAF 自续期循环」的证据。
  if (COND === "theme-sync") {
    const realRafSync = window.requestAnimationFrame.bind(window);
    window.requestAnimationFrame = function (cb) {
      counters.tcRafScheduled += 1;
      if (R.clickPhase === true) counters.tcRafScheduledInClick += 1;
      if ((R.rafCallbackDepth || 0) > 0) {
        // 重入：不能在栈内继续同步展开
        counters.tcRafReentrant += 1;
        return realRafSync(cb);
      }
      counters.tcSyncInvoked += 1;
      R.rafCallbackDepth = 1;
      try { cb(performance.now()); } catch (e) { R.stubLog.push([Date.now(), "raf-sync-throw", String(e && e.message).slice(0, 160)]); }
      finally { R.rafCallbackDepth = 0; }
  ...
  function record(key, ms, detail) {
    const add = (bag) => {
      const b = bag[key] || (bag[key] = { n: 0, total: 0, max: 0, samples: [], detail: null, worstDetail: null });
      b.n += 1; b.total += ms; if (ms > b.max) { b.max = ms; b.worstDetail = detail || null; }
      if (b.detail === null) b.detail = detail || null;
      if (b.samples.length < 400) b.samples.push({ ms: Math.round(ms * 100) / 100, d: detail || null });
    };
    add(counters.http);
    if (R.clickPhase === true) add(counters.httpInClick);
  }
  // 记录「identity」而非只记计数：一次 3.4 s 的请求必须能说清是谁。
  function httpIdentity(method, url, body) {
    let bodyKeys = null, bodyMethod = null, bodyLen = 0;
    if (typeof body === "string") {
      bodyLen = body.length;
      try { const j = JSON.parse(body); bodyKeys = Object.keys(j).slice(0, 8); bodyMethod = j.method || (j.payload && j.payload.method) || null; } catch (e) { }
    }
    return { method: method || null, url: String(url || "").slice(0, 160), bodyLen, bodyKeys, bodyMethod };
  }
  ...
  if (typeof realFetch === "function") {
    window.fetch = function (input, init) {
      ...
      const detail = httpIdentity((init && init.method) || (input && input.method) || "GET", url, body);
      return pr.then((r) => {
        record(key, performance.now() - t, detail);
        return r;
      }, (e) => { record(key, performance.now() - t, detail); throw e; });
    };
  }
  ...
  if (RealXHR) {
    ...
    RealXHR.prototype.send = function (body) {
      ...
      const detail = httpIdentity(self.__regMethod, self.__regUrl, body);
      const done = () => { try { record(key, performance.now() - t, detail); } catch (e) { } };
  ...
  R.probe = {
    snapshot() {
      ...
      sp.effectiveStrict = (function () {
        if (COND === "tp-off" || COND === "tpoff-armed") return counters.tpTokenReadbacks > 0 && counters.tpForcedReplay === counters.tpTokenReadbacks;
        if (COND === "theme-sync") return counters.tcSyncInvoked > 0 && counters.tcSyncInvoked + counters.tcRafReentrant === counters.tcRafScheduled;
      ...
      const summ = (bag) => {
        ...
        for (const k of Object.keys(bag)) {
          ...
          const vv = b.samples.map((x) => x.ms).sort((a, c) => a - c);
          out[k] = { n: b.n, totalMs: Math.round(b.total * 100) / 100,
            p50: vv.length ? vv[Math.floor(0.5 * vv.length)] : null,
            p99: vv.length ? vv[Math.floor(0.99 * vv.length)] : null, max: Math.round(b.max * 100) / 100,
            detail: b.detail, worstDetail: b.worstDetail,
            samples: b.samples.slice(0, 12) };
        }
        return out;
      };
