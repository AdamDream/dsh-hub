...
const perRun = runs.map((r) => {
  ...
  // 停顿成簇结构：把相邻的慢拍（间隔 <2s）归为一簇，并标明该簇相对点击的**起止**。
  // 这是"停顿是否由点击引起"的最干净形态学判据：
  //   簇在**点击之前**就已开始且横跨点击 ⇒ 属既有停顿（点击只是撞上它）；
  //   簇在**点击之后**才开始 ⇒ 才可能是点击触发。
  const slowBeats = beats.filter((b) => b.ms >= STALL_MS);
  const clusters = [];
  for (const b of slowBeats) {
    const last = clusters[clusters.length - 1];
    if (last && b.t0 - last.endT0 < 2000) { last.items.push(b); last.endT0 = b.t0; last.max = Math.max(last.max, b.ms); }
    else clusters.push({ items: [b], startT0: b.t0, endT0: b.t0, max: b.ms });
  }
  for (const c of clusters) {
    c.startDt = c.startT0 - a; c.endDt = c.endT0 - a;
    c.spansClick = c.startDt < 0 && c.endDt >= 0;
    c.startsBeforeClick = c.startDt < 0;
    c.kind = c.startsBeforeClick ? (c.spansClick ? 'STARTS-BEFORE-CLICK-AND-SPANS-IT（既有停顿，点击撞上）' : 'STARTS-BEFORE-CLICK（既有停顿，与点击无关）') : 'STARTS-AFTER-CLICK（候选：可能是点击触发）';
  }
  const newClusterAfterClick = clusters.filter((c) => !c.startsBeforeClick && c.startDt >= 0);
  ...
  return {
    ...
    stalls, bigGaps, gapStallIdentityConsistent: identityConsistent, stallClusters: clusters, newClusterAfterClickCount: newClusterAfterClick.length, rpcInWindow: pairs, attributionBuckets, rpcWindows,
...
const verdict = {
  ...
  T4_repeatability: perRun.length >= 3
    ? `已完成 ${perRun.length} 次「全新页面→点设置」（满足 ≥3）；点击后 500ms 内的 ≥${STALL_MS}ms 命中数=${perRun.map((p) => p.windows.plusMinus500.stalls100_afterClick).join('/')}，`
      + `命中率=${(100 * perRun.filter((p) => p.windows.plusMinus500.stalls100_afterClick > 0).length / perRun.length).toFixed(0)}%；`
      + `点击窗 p95 ÷ 点击前窗 p95 = ${perRun.map((p) => (p.windows.pre.p95 ? (p.windows.plusMinus500.p95 / p.windows.pre.p95).toFixed(2) : 'n/a')).join(' / ')}`
      + `（该比值与宿主负载同步、与是否点击无稳定关系 ⇒ 停顿非点击所致，详见 audit.md §0.1）`
    : 'INCONCLUSIVE(运行次数<3)',
};
