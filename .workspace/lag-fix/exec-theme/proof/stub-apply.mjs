 ...
 *   E7  forced-recalculation count: patched = token-set changes, pristine = one per apply;
 *
...
if (runPatched && runPristine) {
	for (const model of ["transparent", "token"]) {
		...
		const contentKey = (sn) => JSON.stringify([String(sn.active.colorScheme), { ...sn.active.tokens }]);
		const contentChanges = steps.filter((step, i) => i === 0 || contentKey(step.snapshot) !== contentKey(steps[i - 1].snapshot)).length;
		const skipped = patched.steps.filter((s) => s.writes.length === 0 && s.computedStyleCalls === 0).length;
		...
		check("E7" + tag, `forced recalculation count: patched = ${tokenChanges} token-set changes, pristine = one per apply (${steps.length})`,
			pc === tokenChanges && pr === steps.length && applies === contentChanges && skipped === steps.length - contentChanges,
			`patched=${pc} pristine=${pr}; applies=${applies}=content-changes ${contentChanges}; replays skipped=${skipped}/${steps.length - contentChanges}`);
		...
		/* Derived expectation: after the mutation the attribute is absent, so the guard's
		 * `hasAttribute === (scheme === "dark")` is false and the replay must NOT skip. */
		const mutationRemoved = weak.steps[1].dark === true;
		check("E4c" + tag, "an externally removed dark attribute is repaired by the next apply (guard rejects the skip)",
			weak.steps[1].writes.length > 0 && mutationRemoved,
