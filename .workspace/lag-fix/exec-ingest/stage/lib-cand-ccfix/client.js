window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function UsageCard(props) {
			...
			/* dsh-perf-fix R4 v1: 旧响应/卸载保护。RPC 不假设可取消：用单调代次 + alive
			   标志丢弃过期响应；setup 重新置 alive 以兼容 StrictMode 的 setup→cleanup→setup
			   与真实 remount；cleanup 递增代次，使上一轮在飞请求在重挂载后也无法复活。 */
			...
			const loadAll = react.useCallback(async () => {
				...
				try {
					...
					if (current() && calls.slice(0, 6).every((r) => r && r.ok)) {
