...
export function foldCcSource(db, root, { onProgress } = {}) {
	...
	for (const { file, mtime, size, isSubagent } of files) {
		...
		if (state && Number(state.last_offset) > 0) {
			/* dsh-perf-fix CC-cursor v1: 遗留游标（size+1）夹回 state.size —— 既不再跳字节，
			   也让当时被跳过的记录在下一轮被重新读入（自愈）。 */
		...
		/* dsh-perf-fix CC-cursor v1: 文件以换行结尾时，completeLines.join + 换行的写法会比 text
		   多算 1 字节（join 已带回该换行）→ last_offset 写成 size+1 → 下一轮 subarray(size+1)
		   跳过新内容首字节，追加批次第一条记录被静默丢弃。实测复现见
