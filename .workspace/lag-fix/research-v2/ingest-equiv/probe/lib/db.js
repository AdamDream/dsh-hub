...
const dshHomePath = (...x) => "/nonexistent-probe/" + x.join("/");
...
function maxDailyDayMs(db) {
	const now = Date.now();
	globalThis.__cacheStats = globalThis.__cacheStats || { hit: 0, miss: 0, invalidate: 0, reads: [] };
	if (maxDailyDayCache.value !== null && now - maxDailyDayCache.at < MAX_DAILY_DAY_TTL_MS) {
		globalThis.__cacheStats.hit += 1;
		globalThis.__cacheStats.reads.push({ thread: "?", kind: "HIT", value: maxDailyDayCache.value });
		return maxDailyDayCache.value;
	}
	globalThis.__cacheStats.miss += 1;
	const day = queryMaxDailyDay(db);
	const value = day === null ? null : localDayMs(day);
	maxDailyDayCache = { at: now, value };
	globalThis.__cacheStats.reads.push({ kind: "MISS", day, value, fromTable: true });
	return value;
}
/** Invalidate the `MAX(day)` cache (called after each `usage_daily` rewrite). */
function invalidateMaxDailyDayCache() {
	globalThis.__cacheStats = globalThis.__cacheStats || { hit: 0, miss: 0, invalidate: 0, reads: [] };
	globalThis.__cacheStats.invalidate += 1;
