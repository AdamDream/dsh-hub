...
(() => {
  ...
  /*
   * EMPIRICALLY ESTABLISHED index map (scripts/probe-hooks.mjs, raw/probe-hooks.json):
   *  0 useState(open)                    1 useState(activeId)
   *  2 useState(completedOnboarding)     3 useCallback(close)
   *  4 useCallback(openSection)
   *  5 useRef  ) uSES shim internal     6 useMemo ) for useSections
   *  7 useSyncExternalStoreWithSelector  -> rows / sections output (array, 8 rows)
   *  8 useRef  )                        9 useMemo )
   * 10 useSyncExternalStoreWithSelector  -> onboardingSteps output (array, 2 steps)
   * 11 useEffect 12 useRef ) 13 useMemo ) 14 uSES 15 useEffect 16 useDebugValue
   * 17 useSyncExternalStoreWithSelector  -> the sessions boolean
   * 18 useDebugValue 19 useEffect 20 useRef 21 useCallback(completeOnboardingStep)
   * Note the uSES shim expands into useRef+useMemo+useSyncExternalStore+useEffect+
   * useDebugValue, which is why a naive read of the component source gives the wrong
   * indices (5/6/7 instead of 7/10/17).
   */
  const SEL_HOOKS = { 7: 'sections', 10: 'onboardingSteps', 14: 'sessionsInner', 17: 'sessions' };
  ...
  function hookState(f, i) { const h = hookAt(f, i); return h ? h.memoizedState : undefined; }

  /**
   * The two store getSnapshots are memoised on (slot version, locale revision):
   *   sections:        rowsVersion = slots.getVersion("settings.section");  rowsRevision = locale.revision
   *   onboardingSteps: onboardingVersion = slots.getVersion("settings.onboarding")
   * uSES shim internal state ([value, getServerSnapshot] pairs) exposes both the
   * value handed to React and the slot/listener version React is currently
   * subscribed to, so we can read the counters without patching the stores.
   */
  /** Walk up from SettingsRoot to the SlotOutlet ancestor and read its own uSES values. */
  function readAncestorOutlet(f) {
    let n = f.return, depth = 0;
    while (n && depth < 12) {
      if (comp(n) && nameOf(n) === 'SlotOutlet') {
        const a = n.alternate;
        const cur = [hookAt(n, 2), hookAt(n, 3)];
        const alt = a ? [hookAt(a, 2), hookAt(a, 3)] : [null, null];
        const val = (h) => (h ? h.memoizedState : '__none__');
        const label = (v) => v === null ? 'null' : v === undefined ? 'undefined' : (typeof v === 'object' ? 'object' : typeof v + ':' + String(v));
        return {
          found: true, depth,
          pw: signalPw(n), id: signalId(n),
          slotVersion: { cur: label(val(cur[0])), alt: label(val(alt[0])), changedVsAlt: alt[0] ? val(cur[0]) !== val(alt[0]) : null },
          localeRevision: { cur: label(val(cur[1])), alt: label(val(alt[1])), changedVsAlt: alt[1] ? val(cur[1]) !== val(alt[1]) : null },
        };
      }
      n = n.return; depth++;
    }
  ...
  function readVersionCounters(f) {
    return {
      sections: { valueLen: Array.isArray(hookState(f, 7)) ? hookState(f, 7).length : null, version: null, listenerVersion: null },
  ...
  Object.entries = function (o) {
    ...
    try {
      ...
      // ---- slots service detection: the host / ctx.slots face carries getVersion
      if (o && typeof o.getVersion === 'function' && !o.__srSlotWrapped) {
        const ks = e.map((x) => x[0]);
        const interesting = ks.filter((x) => x === 'settings.section' || x === 'settings.onboarding' || x === 'sidebar.settings');
        if (interesting.length) {
          const origGV = o.getVersion;
          o.getVersion = function (key) {
            const v = origGV.call(this, key);
            const had = Object.prototype.hasOwnProperty.call(SR.slotVersionLast, key);
            const prev = SR.slotVersionLast[key];
            const bump = had && prev !== v;
            if (!had) SR.slotVersionLast[key] = v;
            else if (bump) SR.slotVersionLast[key] = v;
            push(SR.slotVersionLog, { t: rt(now()), key, from: had ? prev : null, to: v, commit: SR.counts.commits, kind: had ? (bump ? 'bump' : 'same') : 'first' });
            return v;
          };
          o.__srSlotWrapped = true;
          SR.flags.slotStoreWrapped = true;
          SR.slotStoreKeys = ks.slice(0, 60);
        }
      }
  ...
  SR.dump = () => ({
    ...
    slotVersionLog: SR.slotVersionLog,
    slotStoreKeys: SR.slotStoreKeys,
