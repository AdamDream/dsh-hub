...
/* P2AC 执行档：本副本对原 harness 的唯一实质改动 = 目标指向**候选件**，并把 EXPECTED_SHA256
 * 换成候选件的 sha256；另把 4 条「缺陷必须存在」的断言改为「缺陷必须不存在」，见文件末尾说明。
 * 原 harness（research-v2/semantics/harness-a-p2-stale-row.cjs, sha256 08c04f62…）保持不动，
 * 作为「改前态」物证；本副本 = 「候选态」物证。
 * 候选件 = live(d71a8ca5…) + 审计 §2.2/§2.3 的 P2AC 变换（4 处整行替换，见 exec-p2/apply-P2AC.mjs）。*/
const TARGET = '/home/CNS2026495165/dsh/.workspace/lag-fix/exec-p2/candidate/client.js';
const EXPECTED_SHA256 = '357f1703722464ee7fc40566f13e0ae4dd225131589862c8d1def67deb93d61f';
const OUT_DIR = path.join(__dirname, 'results');
const OUT_JSON = path.join(OUT_DIR, 'p2-stale-row-post-p2ac.json');
...
const requiredAnchors = [
  ...
  ['/* p2ac-fix */', 'P2AC patch marker (the unit that landed)'],
  ['const chainRowIds = new Set();', 'P2AC chain-scope registry declaration (A1)'],
  ['chainRowIds.add(childId);', 'P2AC chain-step registration (A2)'],
  ['if (byId[id] === void 0 && chainRowIds.has(id)) byId[id] = previousProjection.byId[id];', 'chain-scoped carry-forward (A3)'],
  ['reusableByIdKeys.every((id) => Object.prototype.hasOwnProperty.call(byId, id))', 'membership key-set gate'],
...
if (!H.prototype.projectList.toString().includes('/* dsh-perf-fix P2 v1 */')
  || !H.prototype.projectList.toString().includes('const chainRowIds = new Set();')) {
  fatal('runtime projectList lost the P2 marker or the P2AC declaration');
}
 ...
 * ------------------------------------------------------------------ */
{
  ...
  scenarios.push(s1);
  record('S1: NO residue — the address-chain child row is dropped once the address is cleared (P2AC fix)',
    !residue, `byId keys = ${JSON.stringify(Object.keys(after2.byId))}, ids = ${JSON.stringify(after2.ids)}`,
    'executed', true, !residue);
  record('S1: no row is keyed by an id absent from the projection ids list (byId keys == ids)',
    !residue && after2.ids.includes('p') && !after2.ids.includes('child'),
    `ids=${JSON.stringify(after2.ids)} byIdKeys=${JSON.stringify(Object.keys(after2.byId))}`, 'executed', true,
    !residue && after2.ids.includes('p') && !after2.ids.includes('child'));
  record('S1: the real client indexSubagentDescendants(byId) no longer fabricates a child count for p',
    descendants === undefined,
    `indexSubagentDescendants(byId).get("p") = ${JSON.stringify(descendants)} while ids=[p]`, 'executed',
    'undefined (no phantom descendant)', descendants ?? null);
  record('S1: no projection/scope divergence — the child is not published and not eligible, the parent is both',
    residue === false && eligibleChild === false && eligibleParent === true,
    `eligible("child")=${eligibleChild}, eligible("p")=${eligibleParent} (extracted real predicate)`, 'executed',
    'child absent + not eligible; parent present + eligible', { child: eligibleChild, p: eligibleParent });
}
 ...
 * ------------------------------------------------------------------ */
{
  ...
  record('S4: the abandoned child1 row does NOT survive the retarget to child2 (P2AC fix)',
    !both && rowOf(after, 'child1') === undefined && rowOf(after, 'child2') !== undefined,
    `byId keys = ${JSON.stringify(Object.keys(after.byId))}`, 'executed',
    'only child2', { child1: rowOf(after, 'child1') !== undefined, child2: rowOf(after, 'child2') !== undefined });
}
 ...
 * ------------------------------------------------------------------ */
/* P2AC 形态（候选件已落地文本）。本副本做的是**候选态**的反事实对拍：
 * 候选件项目＝正确态（FULL FIX）；负向变体＝只留一半，必须被判 PARTIAL/REGRESSION。*/
const CARRY_ANCHOR = '\t\t\t\t\tfor (const id of Object.keys(previousProjection.byId)) if (byId[id] === void 0 && chainRowIds.has(id)) byId[id] = previousProjection.byId[id];';
const PATCH_A = '\t\t\t\t\t/* candidate P-A variant: the chain filter is replaced by a second, independent walk */ for (const id of Object.keys(previousProjection.byId)) if (byId[id] === void 0 && walkChainIds().has(id)) byId[id] = previousProjection.byId[id];';
...
const REUSE_ANCHOR = '\t\t\t\tconst nextById = previousProjection !== void 0 && reusedEntries === liveKeys.length && reusableByIdKeys.length === liveKeys.length && reusableByIdKeys.every((id) => Object.prototype.hasOwnProperty.call(byId, id)) ? previousProjection.byId : stableById;';
/* 负向变体 P-B：把**成员性闸门整条删掉**（B 那一半缺失，退回"整体复用旧 byId"） */
const REUSE_DELETED = '\t\t\t\tconst nextById = previousProjection !== void 0 && reusedEntries === liveKeys.length ? previousProjection.byId : stableById;';
/* 负向变体 P-C：把**链域登记删掉**（A2 那一半缺失，链域集合恒为空） */
const REGISTER_ANCHOR = '\t\t\t\t\t\tchainRowIds.add(childId);';
const REGISTER_DELETED = '\t\t\t\t\t\t/* negative control P-C: chain registration removed */';
/* 参照：把闸门退回**基数式**（更弱形态，audit §3.2 已对拍为同样 FULL FIX） */
const REUSE_PATCHED = '\t\t\t\tconst nextById = previousProjection !== void 0 && reusedEntries === liveKeys.length && reusableByIdKeys.length === liveKeys.length ? previousProjection.byId : stableById;';
/* 负向变体 P-D：链域过滤退回**无差别回拷**（只留闸门这一半） */
const CARRY_DELETED = '\t\t\t\t\t/* candidate P-D: chain filter removed (blanket carry-forward restored) */ for (const id of Object.keys(previousProjection.byId)) if (byId[id] === void 0) byId[id] = previousProjection.byId[id];';
...
function applyCandidatePatch(id, replacements) {
  ...
  /* P-A 变体需要一个与主 walk 独立的第二遍 chain 推导：把它作为声明插在 if (copiedPrevious) 之前，
   * 这样"等价判据"（audit §3.1 的另一种形态）也能在候选态被对拍。 */
  if (PATCH_A.includes('walkChainIds')) {
    const ifCopied = '\t\t\t\tif (copiedPrevious) {';
    if (text.split(ifCopied).length - 1 !== 1) fatal('P-A variant: `if (copiedPrevious) {` is not unique');
    text = text.replace(ifCopied, '\t\t\t\tconst walkChainIds = () => { const out = new Set(); let a = current === void 0 ? void 0 : currentAddress; while (a !== void 0 && !out.has(a.childSessionId)) { out.add(a.childSessionId); a = this.manager.navigationAddress(a.parentSessionId); } return out; };\n' + ifCopied);
  }
...
const candidates = [
  {
    id: 'P-AC',
    description: 'the candidate态 itself: chain-scoped carry-forward (A1/A2/A3) + membership key-set gate (B) — both halves present',
    klass: applyCandidatePatch('P-AC', [[CARRY_ANCHOR, CARRY_ANCHOR]]),
    expectation: 'FULL: drops the residue and keeps every legitimate row',
    expectedVerdict: 'FULL FIX',
  },
  {
    id: 'P-A',
    description: 'the equivalent form of audit §3.1: the chain filter is computed by a second, independent walk instead of reusing the main walk registry',
    klass: applyCandidatePatch('P-A', [[CARRY_ANCHOR, PATCH_A]]),
    expectation: 'FULL: the equivalent predicate is behaviourally identical on S1-S5',
    expectedVerdict: 'FULL FIX',
  },
  {
    id: 'P-B',
    description: 'negative control: the membership gate is deleted, so the byId identity reuse reverts to the unchecked wholesale reuse',
    klass: applyCandidatePatch('P-B', [[REUSE_ANCHOR, REUSE_DELETED]]),
    expectation: 'PARTIAL: the stale key is republished by the wholesale reuse, so S1 stays stale',
    expectedVerdict: 'PARTIAL',
  },
  {
    id: 'P-D',
    description: 'negative control: the chain filter is removed and the OLD blanket carry-forward is restored (the membership gate is kept)',
    klass: applyCandidatePatch('P-D', [[CARRY_ANCHOR, CARRY_DELETED]]),
    expectation: 'REGRESSION: the blanket carry-forward re-injects the stale key into liveKeys, so S1 stays stale (the gate cannot help — the stale key is already a live key)',
...
for (const candidate of candidates) {
  ...
  /* 候选态判据（与原 harness 有意不同，已在此说明）：
   *   FULL FIX  = 该变体的 patched 结果在**全部 4 项**检查上都成立（S1 干净 + S2/S3/S4 合法行保留）
   *   PARTIAL   = 留住了 S2/S3/S4，但没止住 S1 残留
   *   REGRESSION= 丢掉了合法行（S2/S3/S4 任一项失败）
   * 原 harness 用 `unpatched_ok === false` 作"确实修好了"的依据，是因为它的基准是**缺陷态**；
   * 本副本的基准（候选件）已经是修好的代码，因此基准本身在 S1 上就是 ok，不能再沿用该判据。 */
  const fixesReportedDefect = candidateChecks.every((r) => r.patched_ok === true);
  const keepsControls = candidateChecks.slice(1).every((r) => r.patched_ok === true);
  const verdict = fixesReportedDefect ? 'FULL FIX'
...
results.summary = {
  /* 候选态：S1 的残留行必须消失；S2/S3 的合法行必须保留；S5 自愈仍成立。 */
  p2ac_candidate_no_stale_row: scenarios[0].stale_row_present === false,
  p2ac_candidate_keeps_legitimate_rows: scenarios[1].row !== undefined && scenarios[2].row !== undefined,
