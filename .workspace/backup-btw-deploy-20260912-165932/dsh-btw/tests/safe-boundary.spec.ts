...
import { READ_ONLY_TOOL_SET } from '../src/shared/tool-policy.ts'

function event(seq: number, type: string): SessionEvent {
  return { seq, time: seq, type, data: {} } as unknown as SessionEvent
}

describe('btw retention policy', () => {
  it('has no idle lease: nothing schedules an automatic close', () => {
...
describe('completedTurnSeed', () => {
  ...
  it('returns an empty seed for an empty or open-turn-only history', () => {
