...
assert.equal(typert.TYPERT.invocations.length, 10)
for (const method of ['start', 'read', 'send', 'answer', 'cancel', 'close', 'setModel', 'readImage', 'listTree', 'listProject']) {
  assert.equal(typert.TYPERT.invocations.map(item => item.method).includes(method), true)
}
assert.equal(remote.TYPERT_REMOTE.package, '@local/dsh-btw')
assert.equal(remote.TYPERT_REMOTE.descriptors.length, 10)
assert.equal(remote.TYPERT_REMOTE.descriptors.map(item => item.method).includes('readImage'), true)
assert.equal(remote.TYPERT_REMOTE.descriptors.map(item => item.method).includes('listProject'), true)
