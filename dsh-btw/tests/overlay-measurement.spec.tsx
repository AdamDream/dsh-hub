...
describe('overlay geometry measurement', () => {
  ...
  it('updates placement through ResizeObserver and disconnects every observer on unmount', () => {
    ...
    function Probe({ options }: { options: OverlayPlacementOptions }) {
      ...
      return <div ref={sideRoot} data-dsh-btw-root data-mode={placement.mode} data-left={placement.left} />
    }
    ...
    expect(fixture.slot.querySelector('[data-dsh-btw-root]')?.getAttribute('data-mode')).toBe('right')
    ...
    expect(fixture.slot.querySelector('[data-dsh-btw-root]')?.getAttribute('data-mode')).toBe('bottom-sheet')
    act(() => { root?.unmount() })
    root = undefined
  ...
  it('keeps the real drawer draft while responsive placement changes mode', () => {
    ...
    const sideRoot = fixture.slot.querySelector('[data-dsh-btw-root]')
  ...
  it('keeps the composer usable within a 375px bottom sheet and minimizes from its scrim', () => {
    ...
    const snapshot: SideChatClientState = {
      ...
      seedLength: 0, revision: 0,
      messages: [], partial: '', reasoning: '', running: false,
    }
    ...
    const sideRoot = fixture.slot.querySelector<HTMLElement>('[data-dsh-btw-root]')
    ...
    act(() => { fixture.slot.querySelector<HTMLElement>('[data-dsh-btw-scrim]')?.click() })
    expect(viewStore.get('parent').visible).toBe(false)
    expect(close).not.toHaveBeenCalled()
  })

  it('renders a clear not-open error state with a retry action', () => {
    const fixture = shell()
    const snapshot: SideChatClientState = {
      epoch: 2, phase: 'error', parentSessionId: 'parent' as never,
      error: 'This side conversation is no longer open on the host.',
      seedLength: 0, revision: 0, messages: [], partial: '', reasoning: '', running: false,
    }
    ...
    expect(fixture.slot.textContent).toContain('drawer.error')
    expect(fixture.slot.textContent).toContain('no longer open')
    const retryButton = [...fixture.slot.querySelectorAll('button')].find(button =>
      button.textContent === 'drawer.retry',
    )
