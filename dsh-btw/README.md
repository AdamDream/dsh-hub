...
<h1 align="center">dsh-btw (fork of dsh-side-chat)</h1>

> **Local fork notice.** `@local/dsh-btw` is a local fork of
> [`@lukeknow0/dsh-side-chat`](https://github.com/Lukeknow0/dsh-side-chat) v0.4.0 (MIT, credit and
> thanks to Lukeknow0 and contributors). It keeps the upstream read-only side-conversation design and
> adds three enhancements: **persistent side conversations** (no 30-minute idle expiry, resume across
> DSH restarts through `~/.dsh/btw/index.json`), an **in-progress digest** of what the main agent is
> doing right now, and an **ask-back channel** (`btw_ask_user`) that lets the side assistant ask the
> user clarifying questions inside the panel.
