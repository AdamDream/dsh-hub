...
import { analyzeImages, wrapImageDescriptions, type VisionImageInput } from './vision.ts'

/**
 * Main-session image transform (U-G-1).
 *
 ...
 * carries image parts. This handler replaces every image part with the R1-9
 * text template produced by vision-adam, so the conversation model only ever
 * receives text — any model, regardless of its declared input modalities.
 *
