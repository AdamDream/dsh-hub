...
export interface BtwIndexExtras {
  parentTitle?: string
  parentCwd?: string
  lastPreview?: string
}
...
function parseIndex(raw: string): BtwIndexFile {
  ...
  for (const [parentSessionId, entry] of Object.entries(record.entries as Record<string, unknown>)) {
    ...
    const candidate = entry as {
      childSessionId?: unknown; createdAt?: unknown; lastActiveAt?: unknown
      parentTitle?: unknown; parentCwd?: unknown; lastPreview?: unknown
    }
...
export class BtwRegistry {
  ...
  async set(parentSessionId: string, childSessionId: string, extras?: BtwIndexExtras): Promise<void> {
      ...
      const next: BtwIndexFile = {
        ...
        entries: {
          ...
          [parentSessionId]: {
            ...
            lastActiveAt: now,
            ...(extras?.parentTitle !== undefined ? { parentTitle: extras.parentTitle } : {}),
            ...(extras?.parentCwd !== undefined ? { parentCwd: extras.parentCwd } : {}),
            ...(extras?.lastPreview !== undefined ? { lastPreview: extras.lastPreview } : {}),
          },
        },
      }
  ...
  async touch(parentSessionId: string, extras?: BtwIndexExtras): Promise<void> {
    await this.enqueue(async () => {
      const current = await this.load()
      const previous = current.entries[parentSessionId]
