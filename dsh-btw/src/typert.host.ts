  ...
  model: {
    services: [{
      ...
      members: [
        ...
        { kind: 'method', name: 'setModel', signature: 'setModel(request: SetSideChatModelRequest): Promise<SetSideChatModelResult>', summary: 'Switch the side conversation model for subsequent turns.', jsDoc: '/** Switch the side conversation model for subsequent turns. */' },
        { kind: 'method', name: 'readImage', signature: 'readImage(request: ReadSideChatImageRequest): Promise<ReadSideChatImageResult>', summary: 'Read one attached image back for display.', jsDoc: '/** Read one admitted image back for display (R1-10 thumbnails). */' },
        { kind: 'method', name: 'listTree', signature: 'listTree(request: ListSideChatTreeRequest): Promise<ListSideChatTreeResult>', summary: 'Enumerate side conversations under one session tree.', jsDoc: '/** Enumerate side conversations under one session tree (R0-3). */' },
        { kind: 'method', name: 'listProject', signature: 'listProject(request: ListSideChatProjectRequest): Promise<ListSideChatProjectResult>', summary: 'Enumerate side conversations grouped by the parent working directory.', jsDoc: '/** Enumerate side conversations grouped by the parent working directory (R0-4). */' },
      ],
      types: [],
    }],
    events: [], objects: [],
